...
@RestController
public class PingTask2 extends AssignmentEndpoint {

  @PostMapping("/ping/task2")
  @ResponseBody
  ...
  protected AttackResult furBall(String url) {
    if (url.matches("http://ifconfig\\.pro")) {
      ...
      return success(this).feedback("ping.success").output(html).build();
    }
  ...
  private AttackResult getFailedResult(String errorMsg) {
    return failed(this).feedback("ping.failure").output(errorMsg).build();
  }
}
